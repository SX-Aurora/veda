...
uint64_t callbackIssue(void* data) {
	if(s_callback)
		s_callback((VEDAprofiler_data*)data, VEDA_PROFILER_EVENT_ISSUE);
	return 0;
}
...
uint64_t callbackBegin(void* data) {
	if(s_callback)
		s_callback((VEDAprofiler_data*)data, VEDA_PROFILER_EVENT_BEGIN);
	return 0;
}
...
uint64_t callbackEnd(void* data) {
	if(s_callback)
		s_callback((VEDAprofiler_data*)data, VEDA_PROFILER_EVENT_END);
	free(data);
	return 0;
}
